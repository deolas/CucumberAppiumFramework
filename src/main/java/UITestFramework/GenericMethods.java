...
public class GenericMethods {
    ...
    public boolean waitForVisibility(By targetElement) {
        try {
            ...
            new WebDriverWait(driver, timeOut).until(ExpectedConditions.presenceOfElementLocated(targetElement));
            return true;
    ...
    public void tap(double xPosition, double yPosition) {
    	...
    	//new TouchAction(driver).tap(xPosition, yPosition).waitAction().perform();
    	try {
			Thread.sleep(3000);
		} catch (InterruptedException e) {
			// TODO Auto-generated catch block
		...
		action.tap(PointOption.point(xPosition, yPosition)).perform();*/
    	Dimension dimens = driver.manage().window().getSize();
		int xCoordinate = (int) (dimens.getWidth()*xPosition);
		int yCoordinate = (int) (dimens.getHeight()*yPosition);
		new TouchAction(driver).tap(PointOption.point(xCoordinate, yCoordinate)).waitAction().perform();
    }
    ...
    public void horizontalVerticalSwipe(double fromX, double fromY, double toX, double toY) {
    	Dimension dimens = driver.manage().window().getSize();
		int xCoordinate = (int) (dimens.getWidth());
		int yCoordinate = (int) (dimens.getHeight());
		int Xfrom = (int) (xCoordinate*fromX);
		int Yfrom = (int) (yCoordinate*fromY);
		int Xto = (int) (xCoordinate*toX);
		int Yto = (int) (yCoordinate*toY);
		TouchAction touchAction = new TouchAction(driver);
		touchAction.press(PointOption.point( Xfrom, Yfrom)).waitAction().moveTo(PointOption.point(Xto, Yto)).release().perform();
	}
